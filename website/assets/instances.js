...
function instanceRowHtml(r) {
    return `
                <tr data-export-key="${qEsc(String(r.problem_id) + "::" + r.name)}">
                    ...
                    <td class="num">${(() => { const v = qFmtNum(r.best_value ?? r.bkv); if (v === "-" && r.lambda_count) return `<span class="muted">${qFmtNum(r.lambda_count)} λ</span>`; return r.best_is_optimal && v !== "-" ? `<strong>${v}</strong>` : v; })()}</td>                    <td>${r.best_source_url ? `<a class="dl" href="${qEsc(r.best_source_url)}" target="_blank" rel="noopener">${qEsc(r.best_source_label || r.best_source_type || "source")}</a>` : "-"}</td>
