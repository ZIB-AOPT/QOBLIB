...
The LABS formulation is:

$$
\underset{s\in\{-1,1\}^n}{\min}\;\sum\limits_{k=1}^{n-1}\left(\sum\limits_{i=1}^{n-k}s_i s_{i+k}\right)^2
$$

, where the decision variables are the spin variables $s\in\{-1,1\}^n$.

We note that although PDBO mainly considers binary variables $x\in\{0, 1\}^n$, the primal-dual framework can be natively applied to the spin variables $s\in\{-1,1\}^n$ by introducing the constraints $s_i^2=1$. 
