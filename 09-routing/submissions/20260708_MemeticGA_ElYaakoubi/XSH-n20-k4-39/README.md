...
| ====== |  |
| Total Runtime | 9.233 |
| Time to Solution | 4.456 |
| CPU Runtime | 9.233 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | Single run of a stochastic graph-based genetic algorithm. Known optimal = 717; gap = 2.09%. Time to Solution is the timestamp of the last logged incumbent improvement in that run. Objective is the checker-verified cost of the submitted routes (solver-reported value was 735). No cutoff was set: the solver never reads the known optimal value, so it cannot stop on optimality; the known optimum is quoted here for gap reporting only. Success Threshold is therefore N/A and every feasible run counts as successful. |
