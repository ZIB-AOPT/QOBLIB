...
| ====== |  |
| Total Runtime | 11.795 |
| Time to Solution | 4.582 |
| CPU Runtime | 11.795 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | Single run of a stochastic graph-based genetic algorithm. Known optimal = 610; gap = 6.56%. Time to Solution is the timestamp of the last logged incumbent improvement in that run. Objective is the checker-verified cost of the submitted routes (solver-reported value was 651). No cutoff was set: the solver never reads the known optimal value, so it cannot stop on optimality; the known optimum is quoted here for gap reporting only. Success Threshold is therefore N/A and every feasible run counts as successful. |
