...
| ====== |  |
| Total Runtime | 7.128 |
| Time to Solution | 4.459 |
| CPU Runtime | 7.128 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | Single run of a stochastic graph-based genetic algorithm. Known optimal = 646; gap = 1.7%. Time to Solution is the timestamp of the last logged incumbent improvement in that run. No cutoff was set: the solver never reads the known optimal value, so it cannot stop on optimality; the known optimum is quoted here for gap reporting only. Success Threshold is therefore N/A and every feasible run counts as successful. |
