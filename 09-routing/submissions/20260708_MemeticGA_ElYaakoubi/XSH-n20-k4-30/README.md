...
| ====== |  |
| Total Runtime | 14.947 |
| Time to Solution | 11.572 |
| CPU Runtime | 14.947 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | Single run of a stochastic graph-based genetic algorithm. Known optimal = 685; gap = 0.0%. Time to Solution is the timestamp of the last logged incumbent improvement in that run. No cutoff was set: the solver never reads the known optimal value, so it cannot stop on optimality; the known optimum is quoted here for gap reporting only. Success Threshold is therefore N/A and every feasible run counts as successful. |
