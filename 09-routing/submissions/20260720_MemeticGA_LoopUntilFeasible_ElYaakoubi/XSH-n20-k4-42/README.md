# Submission for XSH-n20-k4-42

This directory contains the submission for the problem **XSH-n20-k4-42**.

| Field | Value 1 |
| --- | --- |
| Problem | XSH-n20-k4-42 |
| Submitter | Othmane El Yaakoubi |
| Affiliation | Independent Researcher |
| Date | 2026-07-20 |
| ====== |  |
| Reference | Code: https://github.com/othma125/heuristicCVRP |
| Best Objective Value | 1011 |
...
| ====== |  |
| Total Runtime | 14.900 |
| Time to Solution | 10.284 |
| CPU Runtime | 14.900 |
| GPU Runtime | 0 |
| QPU Runtime | 0 |
| Other HW Runtime | 0 |
| ====== |  |
| Remarks | Loop-until-feasible variant: each run restarts from a fresh random population until it returns a capacity-feasible solution, so all runs are feasible by construction. Attempts to feasibility over the 5 runs: mean 1.20 (min 1, max 2). Known optimal = 998; best gap = 1.30%. Total Runtime and Time to Solution are wall-clock and include the failed-attempt retries; TTS is measured from the meta-run start to the last incumbent improvement. Every reported value comes from the recorded campaign: the objective time series is logged by the solver at each incumbent improvement, so the final trace point equals the reported Time to Solution. No cutoff was set: the solver never reads the known optimal value, so it cannot stop on optimality; the known optimum is quoted here for gap reporting only. Success Threshold is therefore N/A and every feasible run counts as successful. |
