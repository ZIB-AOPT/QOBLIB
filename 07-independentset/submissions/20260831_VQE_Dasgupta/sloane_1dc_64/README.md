...
| ====== |  |  |
| Remarks | TIMING NOTE: we did not measure CPU-process time (e.g. via time.process_time()) or log CPU utilization separately; only wall-clock time (via time.time()) was recorded, so 'CPU Runtime' is left as N/A and the full wall-clock duration is reported under 'Total Runtime' instead. NOTE ON '# Runs': we report 4 runs (# Runs = 4); each run pools several measurement-shot batches before extracting the best independent set, as described in our reference paper (arXiv:2606.28866). Total Runtime (1173.83s) is the average of these 4 runs' wall-clock times, per CONTRIBUTING.md guidance to report averages across repetitions. PARAMETER TRANSFER RUN: VQE parameters were trained via classical simulation (AerSimulator MPS) and transferred to real IBM Quantum hardware (ibm_marrakesh) for sampling only; no training occurred on hardware. QPU Runtime (8s) is the average actual quantum execution time, excluding queue time. All 4 runs recovered the optimal independent set, size 10, matching QOBLIB's best-known value ('# Feasible Runs' = '# Successful Runs' = 4). 9 distinct maximum-size (10) independent sets have been found across all runs to date (combining this row with the Quantum Simulator row below), all provided as separate files in the solutions/ directory, each tagged with a '# Origin' comment line indicating which row it came from. | TIMING NOTE: we did not measure CPU-process time (e.g. via time.process_time()) or log CPU utilization separately; only wall-clock time (via time.time()) was recorded, so 'CPU Runtime' is left as N/A and the full wall-clock duration is reported under 'Total Runtime' instead. NOTE ON '# Runs': we report 5 runs (# Runs = 5); each run pools several measurement-shot batches before extracting the best independent set, as described in our reference paper (arXiv:2606.28866). Total Runtime (165.54s) is the average of these 5 runs' wall-clock times, per CONTRIBUTING.md guidance to report averages across repetitions; this used a much lower shot count than the Quantum Hardware row (512 vs 4000 shots/iteration during training). All 5 runs recovered the optimal independent set, size 10, matching QOBLIB's best-known value ('# Feasible Runs' = '# Successful Runs' = 5). 9 distinct maximum-size (10) independent sets have been found across all runs to date (combining this row with the Quantum Hardware row above), all provided as separate files in the solutions/ directory, each tagged with a '# Origin' comment line indicating which row it came from. |
