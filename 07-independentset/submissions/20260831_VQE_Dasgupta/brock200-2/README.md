...
| ====== |  |
| Remarks | TIMING NOTE: we did not measure CPU-process time (e.g. via time.process_time()) or log CPU utilization separately; only wall-clock time (via time.time()) was recorded, so 'CPU Runtime' is left as N/A and the full wall-clock duration is reported under 'Total Runtime' instead. NOTE ON '# Runs': we report 5 runs (# Runs = 5). Total Runtime (3282.82s) is the average of these 5 runs' individual wall-clock times (VQE training + sampling/correction for each), per CONTRIBUTING.md guidance to report averages across repetitions. Best set of size 12 matches QOBLIB's best-known (optimal) value. 4 of the 5 runs recovered this optimum ('# Successful Runs' = 4); the 5th run produced a feasible but smaller independent set (size 10), so all 5 count toward '# Feasible Runs' but not '# Successful Runs'. Only 1 distinct maximum-size (12) independent set has been found across all runs to date (a second, smaller set of size 11 was also observed in an earlier run). |
