...
| ====== |  |
| Total Runtime | 232.95 |
| Time to Solution | 232.95 |
| CPU Runtime | 130.78 |
| GPU Runtime | N/A |
| QPU Runtime | 102.17 |
| Other HW Runtime | N/A |
| ====== |  |
| Remarks | Objective is the market-split violation |Ax-b|; 0 = feasible. Quantum read-out taken on ibm_aachen using 76 qubits. QPU Runtime follows the QOBLIB definition (Qiskit Runtime session mode; queue time excluded; inter-job idle attributed to classical runtime): 13 jobs, 1306s of queue excluded; IBM-reported 'actual usage' for the same jobs is 67s. Runtimes are for the single reported configuration; hyperparameter and basis-search time is not included. |
